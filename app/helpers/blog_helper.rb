module BlogHelper
	...
	def popular_blog_posts(limit=3)
		genre_from_host.blog_posts.where(is_popular: true).order("RANDOM()").limit(limit)
	end
	...
	def tag_links(post)
		links = []
		post.gener.blog_tags.each do |tag|
	...
	def recommended_posts(main_post, category=nil, tag=nil, limit=10)
		posts = []
		if main_post.genre.blog_categories.any?
			category ||= main_post.genre.blog_categories.first
		end
		category.blog_posts.where.not(id: main_post.id).each { |post| posts << post }

		if (posts.size < limit)
			if main_post.genre.blog_tags.any?
				tag ||= main_post.genre.blog_tags.first
			end
			excludes = posts.map(&:id)
			excludes << main_post.id
			tag.genre.blog_posts.where.not(id: excludes).each { |post| posts << post }
		end
